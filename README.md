...
| 操作 | 方式 |
|------|------|
| 拖动 | 按住**标题栏**(或收起后的小标签、home 键)拖动,位置自动记住 |
| 隐藏/显示全部 | 单击 🏠 home 键:所有便签整体隐藏;再单击全部恢复(状态会记住) |
...
| 自动收边 | 视口变小(如大屏切到手机)时,跑出屏幕的便签自动收起并贴回最近的屏幕边缘;「新建便签」按钮同理回到默认位置 |
| 删除整张 | 点标题栏 🗑(有确认弹窗) |
