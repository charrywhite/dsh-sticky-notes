...
.dsn-newbox{position:absolute;right:16px;bottom:28px;top:auto;pointer-events:auto;z-index:6;cursor:grab;user-select:none;-webkit-user-select:none;touch-action:none}
.dsn-newbox:active{cursor:grabbing}
.dsn-home{display:flex;align-items:center;justify-content:center;width:48px;height:48px;border-radius:50%;font-size:20px;line-height:1;color:#713f12;background:linear-gradient(180deg,#fef9c3,#fef3c7);border:1px solid rgba(180,140,40,.45);box-shadow:0 10px 24px rgba(60,40,10,.28),0 2px 6px rgba(60,40,10,.15);cursor:pointer;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;transition:transform .12s,filter .12s;padding:0}
.dsn-home:hover{filter:brightness(.96)}
.dsn-home:active{transform:scale(.9)}
.dsn-home.hidden-all{opacity:.88;filter:saturate(.65)}
.dsn-home.hidden-all:hover{filter:saturate(.65) brightness(1.02)}
.dsn-newmenu{position:absolute;bottom:calc(100% + 8px);right:0;display:flex;flex-direction:column;gap:2px;background:#fffdf5;border:1px solid rgba(180,140,40,.35);border-radius:10px;box-shadow:0 10px 26px rgba(60,40,10,.2);padding:4px;min-width:132px;z-index:7}
