...
.dsn-newbox{position:absolute;right:20px;top:30px;pointer-events:auto;z-index:6;width:fit-content;height:fit-content;user-select:none;-webkit-user-select:none}
...
.dsn-home.hidden-all{opacity:.88;filter:saturate(.65)}
.dsn-home.hidden-all:hover{filter:saturate(.65) brightness(1.02)}
...
`
    ...
    function NoteBoard() {
      ...
      /* 长按计时器 + 是否已触发长按(区分「单击隐藏/显示」与「长按新建菜单」) */
      ...
      const onNewDown = (e) => {
        ...
        const onMove = (ev) => {
          ...
          /* 触屏容差 8px:手指微动/边缘误触不算拖动(长按计时不被取消) */
      ...
      return h('div', { className: 'dsn-board' },
        h('div', {
          ...
          h('button', {
            className: 'dsn-home' + (hidden ? ' hidden-all' : ''),
            title: '单击:隐藏/显示全部便签;长按:新建便签(可拖动)',
            onClick: onNewClick,
