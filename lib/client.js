...
.dsn-newbox{position:absolute;right:20px;top:30px;pointer-events:auto;z-index:6;width:fit-content;height:fit-content;display:flex;flex-direction:column;align-items:flex-end;gap:8px;user-select:none;-webkit-user-select:none}
.dsn-homewrap{position:relative}
.dsn-home{display:flex;align-items:center;justify-content:center;box-sizing:border-box;width:48px;height:48px;font-size:22px;line-height:1;color:#713f12;background:linear-gradient(180deg,#fef9c3,#fef3c7);border:1px solid rgba(180,140,40,.45);border-radius:50%;box-shadow:0 10px 24px rgba(60,40,10,.28),0 2px 6px rgba(60,40,10,.15);padding:0;cursor:pointer;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;transition:filter .12s;touch-action:none;pointer-events:auto}
.dsn-home:hover{filter:brightness(.96)}
.dsn-home:active{filter:brightness(.88)}
.dsn-eye{position:absolute;top:-6px;right:-6px;display:flex;align-items:center;justify-content:center;box-sizing:border-box;width:26px;height:26px;font-size:13px;line-height:1;background:linear-gradient(180deg,#fef9c3,#fef3c7);border:1px solid rgba(180,140,40,.45);border-radius:50%;box-shadow:0 6px 16px rgba(60,40,10,.22);padding:0;cursor:pointer;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;transition:filter .12s;touch-action:none;pointer-events:auto}
.dsn-eye:hover{filter:brightness(.96)}
.dsn-eye.off{opacity:.88;filter:saturate(.6)}
.dsn-eye.off:hover{filter:saturate(.6) brightness(1.02)}
.dsn-newmenu{position:absolute;top:calc(100% + 6px);right:0;display:flex;flex-direction:column;gap:2px;background:#fffdf5;border:1px solid rgba(180,140,40,.35);border-radius:10px;box-shadow:0 10px 26px rgba(60,40,10,.2);padding:4px;min-width:132px;z-index:7}
...
`
    ...
    // ---- home 键:全局隐藏/显示所有便签(hidden 只存浏览器,不同步 host/agent) ----
    const HIDDEN_KEY = 'dsh-sticky-notes:hidden:v1'
    function loadHidden() {
    ...
    const NoteCard = React.memo(function NoteCard(props) {
      ...
      /* note.pos 外部变化(host 同步 / 窗口 resize 钳制)时,本地位置跟随更新 */
      React.useEffect(() => { setLocalPos(note.pos) }, [note.pos])
      ...
      /* 单击标题栏本身(标题文字/计数徽章,排除按钮与输入框)→ 收起便签;
         与拖拽共用 suppressClick:拖动过的不算单击,不会误收起 */
      const onHeadClick = (e) => {
        if (suppressClick.current) {
          suppressClick.current = false
          return
        }
        if (e.target && e.target.closest && e.target.closest('.dsn-icon, .dsn-title-input')) return
        onChange(note.id, { collapsed: true })
      }
      ...
      return h('div', { className: 'dsn-root', style: panelStyle },
        h('div', { className: 'dsn-head', onPointerDown: onHeadDown, onPointerMove: onHeadMove, onPointerUp: onHeadUp, onPointerCancel: onHeadUp, onClick: onHeadClick },
          editingTitle
            ? h('input', {
    ...
    function NoteBoard() {
      ...
      const newPosRef = React.useRef(newPos)
      ...
      /* 窗口尺寸变化(缩放/换屏):把拖动过的便签与「新建便签」按钮重新钳回屏幕内。
         边界与拖拽钳制完全一致:左右至少露 48px、顶部 0、底部留 40px;200ms 防抖。
         便签按展开宽度 264 估算(收起的小标签只会更靠内,不会被推得更偏)。 */
      React.useEffect(() => {
        let timer = null
        const clamp = () => {
          const vw = window.innerWidth
          ...
          const next = notesRef.current.map((n) => {
            if (!n.pos) return n
            const x = Math.min(Math.max(n.pos.x, -(264 - 48)), vw - 48)
            const y = Math.min(Math.max(n.pos.y, 0), vh - 40)
            if (x === n.pos.x && y === n.pos.y) return n
            changed = true
            return { ...n, pos: { x, y } }
          })
          if (changed) commit(next)
          const np = newPosRef.current
          if (np) {
            const w = newBoxRef.current && newBoxRef.current.offsetWidth > 0 ? newBoxRef.current.offsetWidth : 120
            const nx = Math.min(Math.max(np.x, -(w - 48)), vw - 48)
            const ny = Math.min(Math.max(np.y, 0), vh - 40)
            if (nx !== np.x || ny !== np.y) {
              const clamped = { x: nx, y: ny }
              newPosRef.current = clamped
              setNewPos(clamped)
              saveNewBtnPos(clamped)
            }
          }
        }
        const onResize = () => {
          if (timer) clearTimeout(timer)
          timer = setTimeout(clamp, 200)
        }
      ...
      return h('div', { className: 'dsn-board' },
        h('div', {
          ...
          h('div', { className: 'dsn-homewrap' },
            ...
            h('button', {
              className: 'dsn-eye' + (hidden ? ' off' : ''),
              title: '隐藏/显示全部便签',
              onClick: onEyeClick,
            }, hidden ? '\u{1F648}' : '\u{1F440}'),
            newMenuOpen
